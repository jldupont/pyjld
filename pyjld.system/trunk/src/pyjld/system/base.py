...
def process_queues(halting, src_agent, agent_name, agent_id, interest_map, responsesInterestList,
    ...
    while True:
        try:
            ...
            _quit=processor(src_agent, agent_name, agent_id, interest_map, responsesInterestList, iq, isq, envelope)
            if _quit:
                break
...
def message_processor(src_agent, agent_name, agent_id, interest_map, responsesInterestList, iq, isq, envelope):
    ...
    _quit, _mtype, handled, snooping=mdispatch(src_agent, agent_id, envelope)
    if _quit:
...
class AgentThreadedBase(Thread, AgentBase):
    ...
    def run(self):
        ...
        _quit=False
        while not self.quit and not _quit:
            try:
                _quit=process_queues(self.halting, self, self.agent_name, self.id, 
                                    self.mmap, self.responsesInterest,
                                    self.iq, self.isq, message_processor)
            except KeyboardInterrupt:
                _quit=True
