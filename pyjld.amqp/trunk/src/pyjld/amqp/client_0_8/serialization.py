...
2009-04-08 Jean-Lou Dupont <python@jldupont.com>
...
class AMQPReaderException(BaseException):
    """
    Exception class for AMQPReader
    """
...
class AMQPReader(object):
    ...
    def __init__(self, source):
        ...
        if isinstance(source, str):
            ...
            raise AMQPReaderException('AMQPReader needs a file-like object or plain string',
                                      'error_amqpreader_invalid_source'
                                      )
    ...
    def read_bit(self):
        ...
        if self.bitcount == 0:
            raise AMQPReaderException("Attempt to read too many bits",
                                      'error_amqpreader_read_bit_underrun'
                                      )
...
class AMQPWriterException(BaseException):
    """
    Base Exception class for AMQPWriter
    """
...
class AMQPWriter(object):
    ...
    def _flushbits(self):
        try:
            ...
            raise AMQPWriterException("error flushing bits",
                                      "error_amqpwriter_flushbits"
                                      )
    ...
    def write_octet(self, n):
        ...
        """
        if (n < 0) or (n > 255):
            raise AMQPWriterException('Octet out of range 0..255',
                                      'error_amqpwriter_invalid_octet',
                                      value=n)
        self._flushbits()
        try:
            self.out.write(pack('B', n))
        except Exception,e:
            raise AMQPWriterException("error writing an octet from [%d]" % n,
                                      "error_amqpwriter_write_octet",
                                      value=n
                                      )
    ...
    def write_short(self, n):
        ...
        """
        if (n < 0) or (n > 65535):
            raise AMQPWriterException('Short out of range 0..65535',
                                      'error_amqpwriter_invalid_short',
                                      value=n)
        self._flushbits()
        try:
            self.out.write(pack('>H', n))
        except Exception,e:
            raise AMQPWriterException('error writing short from [%d]' % n,
                                      'error_amqpwriter_writing_short',
                                      value=n)
    ...
    def write_long(self, n):
        ...
        """
        if (n < 0) or (n >= (2**32)):
            raise AMQPWriterException('Long out of range 0..2**31-1',
                                      'error_amqpwriter_invalid_long',
                                      value=n)
        self._flushbits()
        try:
            self.out.write(pack('>I', n))
        except Exception,e:
            raise AMQPWriterException('error writing long from [%d]' % n,
                                      'error_amqpwriter_writing_long',
                                      value=n)
    ...
    def write_longlong(self, n):
        ...
        """
        if (n < 0) or (n >= (2**64)):
            raise AMQPWriterException('LongLong out of range 0..2**64-1',
                                      'error_amqpwriter_invalid_longlong',
                                      value=n)
        self._flushbits()
        try:
            self.out.write(pack('>Q', n))
        except Exception,e:
            raise AMQPWriterException('error writing longlong from [%d]' % n,
                                      'error_amqpwriter_writing_longlong',
                                      value=n)
    ...
    def write_shortstr(self, s):
        ...
        if isinstance(s, unicode):
            s = s.encode('utf-8')
        if len(s) > 255:
            raise AMQPWriterException('String too long 0..255',
                                      'error_amqpwriter_invalid_short_string',
                                      value=s)
    ...
    def write_longstr(self, s):
        ...
        try:
            ...
            raise AMQPWriterException('error encoding UTF-8 longstring',
                                      'error_amqpwriter_encoding_longstr'
                                      )
    ...
    def write_table(self, d):
        ...
        try:
            ...
            raise AMQPWriterException('error writing table',
                                      'error_amqpwriter_writing_table',
                                      value=d
                                      )
    ...
    def write_timestamp(self, v):
        ...
        try:
            ...
            raise AMQPWriterException('error writing timestamp from [%d]' % v,
                                      'error_amqpwriter_writing_timestamp',
                                      value=v
                                      )
